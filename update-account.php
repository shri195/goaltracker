    ...
    <div class="admin-content">
      <div class="card">
        <div class="card-header">
          <h2 class="d-inline">Update Account</h2>
        ...
        <div class="card-body position-relative">
          <?php 
            $cat_id = $_GET['vcid'];
            $db = new Database();
            $db->select('accounts','*',null,"id=$cat_id",null,null);
            $result = $db->getResult();
            if(count($result) > 0){
              foreach($result as $row){
          ?>
          <form class="yourform" id="update-account" action="" method="post" autocomplete="off">
              <div class="form-group">
                  <label>Name</label>
                  <input type="hidden" name="cat_id" value="<?php echo $row['id']; ?>" required>
                  <input type="text" class="form-control cat_name" placeholder="Name" name="cat_name" value="<?php echo $row['name']; ?>" required>
              </div>
              <div class="form-group">
                  <label>Status</label>
